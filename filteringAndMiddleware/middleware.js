...
const camelCase = (rawWord) => {
    return rawWord.trim().split(' ').map(w => w.toLowerCase()).map(w => w = w[0].toUpperCase() + w.substring(1)).join(' ')
}
...
middleware.setUserToRequest = async (req, res, next) => {
    ...
    if (!authHeader) {
        return res
            .sendStatus(errors.unauthorized.code)
    }
...
middleware.setWantedPermission = (req, res, next) => {
    const routerPath = [config.server.urlPrefix,...req.baseUrl.substring(config.server.urlPrefix.length).split('/').filter(item => item != '')]
    const path = req.route.path
    const routerPermissions = getRouterPermissions(routerPath)[path]
    const permissions = {}
    permissions.roles = routerPermissions.filter(p => p.role != undefined).map(p => p.role)
    permissions.field = routerPermissions.filter(p => p.field != undefined).map(p => p.field)
    permissions.field = permissions.field.length != 0 ? permissions.field[0] : undefined
    if (permissions.roles.length == 0 && permissions.field == undefined) {
        permissions.authenticated = routerPermissions.find(p => p.authenticated != undefined)
        permissions.authenticated = permissions.authenticated ? permissions.authenticated.authenticated : false
    } else {
...
middleware.checkPermissions = async (req, res, next) => {
    ...
    if (req.permissions.roles.length != 0 && req.permissions.field != undefined) {
        ...
        req.permissions.fieldWanted = req.permissions.rolePassed ? false : true
    } else if (req.permissions.roles.length != 0 && req.permissions.field == undefined) {
        req.permissions.rolePassed = hasCommons(req.permissions.roles, req.roles)
        ...
        req.permissions.fieldWanted = req.permissions.field == undefined ? false : true
...
middleware.ownerOrHasPermissions = (req, object) => {
    if (req.permissions.authenticated) { // Если требуется авторизация
        if (!req.permissions.roleWanted && !req.permissions.fieldWanted) { // Если не требуется роль, и не требуется соответствие полю
            return true
        }
        if (req.permissions.roleWanted && req.permissions.rolePassed) { // Если требуется роль, то проверяем, прошла ли она проверку
            return true
        }
        if (req.permissions.fieldWanted) {
            if (object._options.attributes.includes(req.permissions.field) && object[req.permissions.field] == req.user.id) { // Если требуется соответствие полю (например ownerId), то проверяем
                return true
            }
            if (!object._options.attributes.includes(req.permissions.field)) { // Если объект не содержит искомого поля
                return true
            }
        }
        return false
    } else { // Если авторизация не требуется 
        return true
    }
}

middleware.permissions = [middleware.setWantedPermission, middleware.setUserToRequest, middleware.setRolesToRequest, middleware.checkPermissions]
